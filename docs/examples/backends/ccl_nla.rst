...
This page demonstrates how the PLIMA nonlinear linear alignment amplitude
prescriptions can be used with the Core Cosmology Library (CCL).

PLIMA supplies the intrinsic-alignment amplitude, while CCL supplies the
cosmology, nonlinear matter power spectrum, weak-lensing kernels, and angular
projection.

Constant NLA model
------------------

The constant NLA model uses a redshift-independent intrinsic-alignment
amplitude.

.. plot::
   :include-source:
   :caption: Nonlinear-alignment angular spectra for a constant IA amplitude.
   ...
   nonlinear_power = cosmology.get_nonlin_power()
   ...
   ax.set_title("Constant NLA angular spectra")
   ax.legend(frameon=False)

   fig.tight_layout()

Redshift-dependent NLA model
----------------------------

The ``nla_z`` model introduces a linear dependence on scale factor.

.. plot::
   :include-source:
   :caption: Effect of redshift evolution on the NLA angular spectrum.
   ...
   ax.set_xlabel(r"Angular multipole $\ell$")
   ax.set_ylabel(r"$\ell(\ell+1)C_\ell/(2\pi)$")
   ax.set_title("Redshift-dependent NLA model")
   ax.legend(frameon=False)

   fig.tight_layout()

Halo-mass-dependent NLA model
-----------------------------

The ``nla_m`` model weights the IA amplitude by the red-galaxy fraction and
effective halo mass of the source population.

.. plot::
   :include-source:
   :caption: NLA spectrum with a halo-mass-dependent amplitude.

   import matplotlib.pyplot as plt
   import numpy as np
   import pyccl as ccl

   from plima.models.nla import nla_mass_amplitude
   ...
   ax.set_xlabel(r"Angular multipole $\ell$")
   ax.set_ylabel(r"$\ell(\ell+1)C_\ell/(2\pi)$")
   ax.set_title("Halo-mass-dependent NLA model")
   ax.legend(frameon=False)

   fig.tight_layout()

Luminosity-function-dependent NLA model
---------------------------------------

The ``nla_lf`` model combines redshift evolution, red-galaxy fraction, and a
luminosity-weighted IA contribution.

.. plot::
   :include-source:
   :caption: NLA spectrum with a luminosity-function-dependent amplitude.
   ...
   ax.plot(
       ...
       color=PLIMA_RED,
       linewidth=2.8,
       label="Luminosity-dependent NLA",
   )
   ...
   ax.set_xlabel(r"Angular multipole $\ell$")
   ax.set_ylabel(r"$\ell(\ell+1)C_\ell/(2\pi)$")
   ax.set_title("Luminosity-function-dependent NLA model")
   ax.legend(frameon=False)
